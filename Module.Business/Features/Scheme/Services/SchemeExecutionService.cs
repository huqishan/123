...
public static class SchemeExecutionService
{
    ...
    /// <summary>
    /// 执行前发布事件，可通过 <see cref="SchemeExecutionEventArgs.Cancel"/> 取消执行操作
    /// </summary>
    ...
    /// <summary>
    /// 根据工位号和方案名称获取执行，同一个工位同时只能有一个执行实例
    /// </summary>
    ...
    public static SchemeExecutionControlActionResult Pause(string stationNo)
    {
        ...
        if (pausedCount > 0)
        {
            PublishExecutionControlStatus(normalizedStationNo, "暂停", contexts.First().Key.SchemeName, "已发送暂停指令");
        }
    ...
    public static SchemeExecutionControlActionResult Continue(string stationNo)
    {
        ...
        if (resumedCount > 0)
        {
            PublishExecutionControlStatus(normalizedStationNo, "继续", contexts.First().Key.SchemeName, "已恢复执行");
        }
    ...
    public static SchemeExecutionControlActionResult Stop(string stationNo)
    {
        ...
        string normalizedStationNo = NormalizeRequiredText(stationNo);
        PublishExecutionControlStatus(normalizedStationNo, "终止", contexts.First().Key.SchemeName, "已通知终止执行");
        ...
        SchemeProfile scheme)
    {
        ...
        if (beforeSchemeArgs.Cancel)
        {
            ...
            PublishSchemeStatus(context.Key.StationNo, "取消", scheme, "Scheme execution was canceled before start.", false);
            return SchemeExecutionResult.CreateCanceled(
        ...
        Raise(SchemeExecuting, SchemeExecutionEventArgs.CreateScheme(
            ...
            startTime: startTime));
        PublishSchemeStatus(context.Key.StationNo, "执行中", scheme, $"开始执行方案：{scheme.SchemeName}");
        ...
        int workStepIndex)
    {
        ...
        {
            DateTime canceledAt = DateTime.Now;
            PublishSchemeStatus(context.Key.StationNo, "取消", scheme, "Work step execution was canceled before start.", false);
            return SchemeExecutionResult.CreateCanceled(
        ...
        PublishSchemeStatus(
            ...
            "执行中",
            scheme,
            $"执行步骤 {workStepIndex}：{schemeStep.StepName}");
        ...
        int stepIndex)
    {
        ...
        {
            DateTime canceledAt = DateTime.Now;
            PublishSchemeStatus(context.Key.StationNo, "取消", scheme, "Step execution was canceled before start.", false);
            return SchemeExecutionResult.CreateCanceled(
        ...
        PublishSchemeStatus(
            ...
            "执行中",
            scheme,
            $"执行操作 {workStepIndex}.{stepIndex}：{operation.DisplayText}");
        ...
        foreach (ReturnValue rv in operation.ReturnValues)
        {
            if (!string.IsNullOrWhiteSpace(rv.ReturnParameterName))
            {
                returnValues[rv.ReturnParameterName.Trim()] = resultText;
            }
        }

        foreach (ReturnValue rv in operation.ReturnValues.Where(rv => rv.IsShowView))
        {
            Module.Business.Business.System.SendDataToView(
                rv.ReturnParameterName,
                rv.JudgeType,
                resultText,
                rv.JudgeValue);
        }
        ...
        IReadOnlyDictionary<string, string> returnValues)
    {
        ...
        bool result = methodName switch
        {
            ...
            "不等于判断" => !TextEquals(GetValue(values, 0), GetValue(values, 1)),
        ...
        IReadOnlyDictionary<string, string> returnValues)
    {
        ...
        if (TryResolveProtocolCommand(operation.PCommandName, operation.PCommandName, parameterValues, out string message))
        {
            return message;
        }
    ...
    {
        return string.Equals(operation.OperationObjectName?.Trim(), SystemOperationObjectName, StringComparison.OrdinalIgnoreCase) ||
               string.Equals(operation.OperationObjectName?.Trim(), "系统", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsLuaOperation(WorkStepOperation operation)
    {
        return string.Equals(operation.OperationObjectName?.Trim(), LuaOperationObjectName, StringComparison.OrdinalIgnoreCase);
    }
